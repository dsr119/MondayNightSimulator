...
export const divisionCount=()=>3;
// Partial official assignments must be completed, never silently discarded.
export const randomDivisionsPending=state=>state.teams.length>0&&state.teams.every(t=>!t.division?.trim());
export function assignSimulationDivisions(state,rng){
 const teams=state.teams.map(t=>({...t}));
 if(!randomDivisionsPending(state))return teams;
 for(let i=teams.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[teams[i],teams[j]]=[teams[j],teams[i]];}
 teams.forEach((t,i)=>{t.division='Provisional '+(i%divisionCount(state)+1);});
 return teams;
}
export function forecastIssues(state){
 const issues=validation(state),divs=[...new Set(state.teams.map(t=>t.division))],provisional=randomDivisionsPending(state),count=divisionCount(state);
