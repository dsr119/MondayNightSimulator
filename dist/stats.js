...
export function playerStats(state,b){
 ...
 return {id:b.id,name:b.name,entering:b.entering,games:games.length,pins:sum(games),average:avg,improvement:avg===null?null:avg-b.entering,high:games.length?Math.max(...games):null,low:games.length?Math.min(...games):null,highSeries:series.length?Math.max(...series):null,averageSeries:series.length?mean(series):null,sd:games.length>1?Math.sqrt(sum(games.map(g=>(g-avg)**2))/(games.length-1)):null,last3:recent(3),last6:recent(6),last9:recent(9),g200:games.filter(g=>g>=200).length,g250:games.filter(g=>g>=250).length,g300:games.filter(g=>g===300).length,sessions:s};
}
