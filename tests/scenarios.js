...
export function scenarios(){
 ...
 function fixture(count=8){
  ...
  for(let n=1;n<=count;n++){const players=[];for(let i=1;i<=5;i++){const id=n+'-'+i;players.push(id);s.bowlers.push({id,name:'Bowler '+id,entering:200,team:n,history:[]});}s.teams.push({number:n,name:'Team '+n,division:String((n-1)%3+1),players});}
 ...
 test('Twenty-team three-division format',()=>{const s=fixture(20);s.rules.fillEightWithWildcards=true;eq(forecastIssues(s),[],'Three divisions ready');const r=simulate(s,2,99);for(let h=0;h<2;h++)assert(Math.abs(r.teams.reduce((n,t)=>n+t.halves[h],0)-3)<1e-8,'Three half winners');});
 ...
 test('Pending divisions draw balanced groups without changing official data',()=>{
  const s=fixture(20);s.teams.forEach(t=>t.division='');const before=JSON.stringify(s),rng=random(42);
  const a=assignSimulationDivisions(s,rng),b=assignSimulationDivisions(s,rng);
  eq([...new Set(a.map(t=>t.division))].map(d=>a.filter(t=>t.division===d).length),[7,7,6],'Balanced');
  eq(new Set(a.map(t=>t.number)).size,20,'Each team once');
  assert(JSON.stringify(a)!==JSON.stringify(b),'Fresh draw');
  eq(assignSimulationDivisions(s,random(42)),a,'Reproducible draw');
  eq(JSON.stringify(s),before,'Original unchanged');
  s.teams.forEach((t,i)=>t.division=String(i%3+1));
  eq(assignSimulationDivisions(s,random(42)),s.teams,'Official groups preserved');
 });
 test('Random division forecasts reconcile, preserve actuals and switch to official',()=>{
  const s=importScores(fixture(),scoreCSV());s.teams.forEach(t=>t.division='');
  for(const sch of s.schedule)if([10,13,16,26,29,32].includes(sch.week)){sch.kind='position';sch.pairs=[];}
  const before=JSON.stringify(s),a=simulate(s,3,123),b=simulate(s,3,123);
  eq(forecastIssues(s),[],'Pending allowed');eq(a.divisionMode,'random','Provisional label');
  eq(a.teams,b.teams,'Seeded random divisions');eq(a.weekly,b.weekly,'Seeded opponents');
  eq(a.weekly[0].teams.map(t=>t.points),Array(8).fill(15),'Actual scores fixed');
  eq(JSON.stringify(s),before,'Roster and scores unchanged');
  for(let h=0;h<2;h++)assert(Math.abs(a.teams.reduce((n,t)=>n+t.halves[h],0)-3)<1e-8,'Half winners');
  assert(Math.abs(a.teams.reduce((n,t)=>n+t.playoffs,0)-8)<1e-8,'Eight berths');
  assert(Math.abs(a.teams.reduce((n,t)=>n+t.champion,0)-1)<1e-8,'One champion');
  s.teams[0].division='1';assert(forecastIssues(s).some(i=>i.includes('Complete all official')),'Partial blocked');
  s.teams.forEach((t,i)=>t.division=String(i%3+1));
  assert(inputKey(s)!==a.inputKey,'Official assignments invalidate run');
  eq(simulate(s,1,123).divisionMode,'official','Automatic official mode');
 });
